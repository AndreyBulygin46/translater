...
bt_translate = Button(win, text='Перевести', width=25, height=2, command=lambda: (text_output.delete('1.0', END), text_output.insert(END, translate(text_input.get('1.0', END)))))
bt_translate.place(x=10, y=370)

bt_copy = Button(win, text='Скопировать', width=25, height=2, command=lambda: pc.copy(text_output.get('1.0', END)))
...
kb.add_hotkey('alt+x', lambda: (show_window(), text_input.delete('1.0', END), text_input.insert(END, pc.paste()), text_output.delete('1.0', END), text_output.insert(END, translate(text_input.get('1.0', END)))))
kb.add_hotkey('alt+3', lambda: text_output.insert(END, translate(text_input.get('1.0', END))))
